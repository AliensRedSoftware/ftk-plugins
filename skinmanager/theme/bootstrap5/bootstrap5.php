...
class bootstrap5{
	...
	public function btn($opt){
		...
		if($modal){
			$tag.="data-bs-toggle=\"modal\" data-bs-target=\"#$modal\" ";
		}
		$tag.=$css;
		$tag.=$enabled;
		$tag=trim($tag);
	...
	public function dropdown(array $opt=[string=>['css','theme','content'=>[],'item'=>[string=>['href','modal']]]]){
		foreach($opt[0] as $title=>$item){
			...
			if(empty($content)){
				$output.="<div class=\"btn-group\" $css><button type=\"button\" class=\"btn btn-$theme dropdown-toggle\" data-toggle=\"dropdown\" data-bs-toggle=\"dropdown\" aria-expanded=\"false\" style=\"width:inherit;\">$title</button><ul class=\"dropdown-menu panel-default\">$items</ul></div>";
			}else{
            	$output.="$content<div class=\"btn-group\" $css><button type=\"button\" class=\"btn btn-$theme dropdown-toggle\" data-toggle=\"dropdown\" data-bs-toggle=\"dropdown\" aria-expanded=\"false\" style=\"width:inherit;\">$title</button><ul class=\"dropdown-menu panel-default\">$items</ul></div>";
			}
		}
		return$output;
	}
	...
	public function item($title,$href,$modal){
		...
		if($modal){
			...
			if(x::startWith('#', $href)){
				$href=substr($href,1);
				$tag.="data-bs-toggle=\"modal\" data-bs-target=\".$href\" ";
			}
		}else{
			$tag.="href=\"$href\" ";
		}
...
return  x::div([
	...
	public function panelToArray($opt){
		...
		foreach($opt['data'] as $title =>$content){
			$i++;
			if(count($opt['data'])==$i){
				$panel.=self::panel(['title'=>$title,'content'=>$content,'last'=>true,'css'=>$css]);
			}else{
	...
	public function lightbox($opt){
        ...
        if($opt['box']){//BOX
        	...
        	return"<a $style_lightbox href=\"$src\" rel=\"shadowbox[img]\"><img $style_img data-src=\"$src\"/></img></a>";
        }
		return"<img $style_img data-src=\"$src\"/></img>";
	}
	/**
	 * Возвращает тему
